...
usage() {
  ...
  printf "\t%s\n\n" "bash path/to/$SCRIPT --tag <TAG> --mode <MODE> [--fix <FIX> --dtype <DTYPE> --minos <MINOS> --nom --help]"
  printf "%s\n\n" "Options:"
  #--tag TAG
  printf "\t%-20s\n" "--tag TAG"
...
if $do_Comb; then
  ...
  for dtype in $DTYPE; do
    ...
    cmd="quickFit -f workspace/hbbj/${title}/${title}_model_${dtype}_${TAG}.root -d combData -p mu_Zboson=${mu_Zboson},mu_Wboson=${mu_Wboson},mu_Higgs=${mu_Higgs},mu_ttbar=${mu_ttbar} -o output/${outname} --savefitresult 1 --saveWS true --ssname quickfit --minStrat ${minStrat} --minTolerance ${minTolerance} --hesse ${hesse} --minos ${minos} ${fix}"
    echo $cmd
    eval $cmd
  done
fi
